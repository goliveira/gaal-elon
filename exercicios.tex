...
Como $\overrightarrow{CA}$ e $\overrightarrow{CB}$ são linearmente independentes, podemos representar $\overrightarrow{PA}$, $\overrightarrow{PB}$ e $\overrightarrow{PC}$ como combinações lineares de $\overrightarrow{CA}$ e $\overrightarrow{CB}$.
Escrevemos
\begin{align*}
...
\]
Portanto, para $\beta \in \R$, obtemos
\[
...
\]
Como $\overrightarrow{PA} + \overrightarrow{PB} + \overrightarrow{PC} = 0$, segue que
\[
  (3 \beta (1 - \alpha) + 3\alpha - 2) \overrightarrow{CA} + (1 - 3 \beta) \overrightarrow{CB} = 0.
\]
Visto que $\overrightarrow{CA}$ e $\overrightarrow{CB}$ são linearmente independentes, essa igualdade implica
\[
  (3 \beta(1 - \alpha) + 3\alpha - 2) = 0 \qquad \text{e} \qquad 1 - 3 \beta = 0.
\]
(Veja o Exercício 1 da Seção 15.)
A segunda equação implica $\beta = 1/3$.
Pela primeira equação temos $3(1/3)(1 - \alpha) + 3\alpha - 2 = 0$, ou seja $\alpha = 1/2$.
